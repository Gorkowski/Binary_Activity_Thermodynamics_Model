function [Csat_approx]=VBS_equilibration_extractCsat_withLLEpartition_KGv2(Cp_j_VBSold, Cstar_j_VBSold, ...
     aw_measurment, molecular_weight, O2C_values, H2C_values,  McGlashan_mode, McGlashan_refinement_mode,N2C_values_denistyOnly )
%%
...
Molar_mass_raitos=18.016./molecular_weight;

aw_vec=ones(S(1,1),1).*aw_measurment;

McGlashan_fit_tolerance=10^-8;

[~, mole_frac_org_beta] = inverted_NNMcGlashan_v8(O2C_values,H2C_values, Molar_mass_raitos, aw_vec, McGlashan_mode);
...
for i=1:S(1,1)
        ...
        if strcmpi(McGlashan_refinement_mode,'interpolate')
              ...
              [~, func2, ycalc1, activity_coefficient_beta(i,1), activity_calc2_water, activity_calc2_beta, mass_fraction_water_beta(i,1), mass_fraction2,~,~, ~ ]=...
                ...
                Molar_mass_raitos(i,1),McGlashan_mode,[], [McGlashan_refinement_mode,'beta'], aw_vec(i,1), N2C_values_denistyOnly);
  
        else
            [~, func2, ycalc1, activity_coefficient_beta(i,1), activity_calc2_water, activity_calc2_beta, mass_fraction_water_beta(i,1), mass_fraction2,~,~, ~ ]=...
                BAT_activity_calc_with_refinement_v1(mole_frac_org_beta(i,1), O2C_values(i,1), H2C_values(i,1),...
                Molar_mass_raitos(i,1),McGlashan_mode,[], McGlashan_refinement_mode,  aw_vec(i,1), N2C_values_denistyOnly);
        end
end
